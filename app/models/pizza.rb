class Pizza < ApplicationRecord
	...
	def unique_crust_and_toppings_combination
		...
		current_toppings = toppings.map(&:topping_name).sort
		...
		similar_pizzas.each do |pizza|
			...
			# Rails.logger.debug "SIMILAR PIZZA: #{pizza.toppings.pluck(:topping_name).sort}"
			
			# Compares toppings between the current pizza (current_toppings) and similar pizzas (pizza.toppings.pluck(:topping_name).sort)
